    ...
    for c in args.chroms:
        ...
        for block_nums in pyit.grouper(chunksize, range(len(chr_blocks))):
            ...
            for i, start_ind, end_ind in zip(
                ...
                VTahat = V.T.dot(ahat)
                VTbhat = V.T.dot(bhat)
                VabsTahat = Vabs.T.dot(ahat)
                VabsTbhat = Vabs.T.dot(bhat)
                for j,n in enumerate(names):
                    vTahat = VTahat[j]; vTbhat = VTbhat[j]
                    vTahat_stdp = perm(V[:,j],ahat); vTbhat_stdp = perm(V[:,j],bhat)
                    vTahat_stds = signperm(V[:,j], ahat)
                    vTbhat_stds = signperm(V[:,j], bhat)
                    vabsTahat = VabsTahat[j]; vabsTbhat = VabsTbhat[j]
                    vabsTahat_stdp = perm(Vabs[:,j],ahat);vabsTbhat_stdp=perm(Vabs[:,j],bhat)
                    vabsTahat_stds = signperm(Vabs[:,j], ahat)
                    vabsTbhat_stds = signperm(Vabs[:,j], bhat)
                    blockresults = blockresults.append({
                        ...
                        'Lambda':args.Lambda,
                        'vTahat':vTahat,
                        'vTahat_stdp':vTahat_stdp,
                        'vTahat_stds':vTahat_stds,
                        'vTbhat':vTbhat,
                        'vTbhat_stdp':vTbhat_stdp,
                        'vTbhat_stds':vTbhat_stds,
                        'vabsTahat':vabsTahat,
                        'vabsTahat_stdp':vabsTahat_stdp,
                        'vabsTahat_stds':vabsTahat_stds,
                        'vabsTbhat':vabsTbhat,
                        'vabsTbhat_stdp':vabsTbhat_stdp,
                        'vabsTbhat_stds':vabsTbhat_stds,
                        'suppVo':(Vo[:,j]!=0).sum(),
